...
import { BarChart3, Users, Activity, Bell, Plus, Edit, Save, X, LogOut, User, Check, Trash2, Settings, Download, TrendingUp, TrendingDown, AlertTriangle, Droplets, UserPlus, ClipboardList, RefreshCw } from 'lucide-react';
...
const ProfessionalDashboard = () => {
  ...
  const getStatusColor = (s) => ({ critical: 'bg-red-500', low: 'bg-yellow-500', stable: 'bg-green-500', high: 'bg-blue-500' }[s] || 'bg-gray-500');
  const getAppStatusColor = (s) => ({ pending: 'text-yellow-600 bg-yellow-100', accepted: 'text-green-600 bg-green-100', rejected: 'text-red-600 bg-red-100', completed: 'text-blue-600 bg-blue-100' }[s] || 'text-gray-600 bg-gray-100');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8"><h1 className="text-2xl font-bold text-gray-900 mb-2">{hemocenter?.name}</h1><p className="text-gray-600">Gerencie o estoque de sangue e as candidaturas de doadores</p></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
              {[
                { icon: Activity, color: 'red', label: 'Estoque Crítico', value: statistics.critical },
                { icon: TrendingDown, color: 'yellow', label: 'Estoque Baixo', value: statistics.low },
                { icon: TrendingUp, color: 'green', label: 'Estoque Estável', value: statistics.stable },
                { icon: BarChart3, color: 'blue', label: 'Total Unidades', value: statistics.total },
                { icon: Users, color: 'purple', label: 'Doadores', value: donors.length }
              ].map(card => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Card de Gestão de Estoque com Prioridade Visual */}
          ...
          <div className="bg-white rounded-lg shadow-sm"><div className="p-6 border-b border-gray-200"><h2 className="text-lg font-semibold text-gray-900">Candidaturas de Doadores</h2></div><div className="p-6 space-y-4 max-h-96 overflow-y-auto">
            {applications.map(app => (<div key={app.id} className="border border-gray-200 rounded-lg p-4"><div className="flex justify-between items-start mb-3"><div><h3 className="font-semibold text-gray-900">{app.user_profiles?.name}</h3><p className="text-sm text-gray-600">Tipo: {app.user_profiles?.donor_profiles[0]?.blood_type || 'N/A'}</p><p className="text-xs text-gray-500">{new Date(app.application_date).toLocaleDateString('pt-BR')}</p></div><span className={`px-2 py-1 rounded-full text-xs font-medium ${getAppStatusColor(app.status)}`}>{app.status}</span></div>{app.status === 'pending' && (<div className="flex space-x-2"><button onClick={() => updateApplicationStatus(app.id, 'accepted')} className="flex-1 bg-green-600 text-white py-1 px-2 rounded text-xs flex items-center justify-center"><Check className="h-3 w-3 mr-1" />Aceitar</button><button onClick={() => updateApplicationStatus(app.id, 'rejected')} className="flex-1 bg-red-600 text-white py-1 px-2 rounded text-xs flex items-center justify-center"><X className="h-3 w-3 mr-1" />Rejeitar</button></div>)}</div>))}
          </div></div>
        </div>
        <div className="mt-8 bg-white rounded-lg shadow-sm"><div className="p-6 border-b border-gray-200 flex justify-between items-center"><h2 className="text-lg font-semibold text-gray-900">Campanhas de Doação</h2><button onClick={() => setIsCampaignModalOpen(true)} className="bg-primary-600 text-white px-4 py-2 rounded-lg flex items-center"><Plus className="h-4 w-4 mr-2" />Nova Campanha</button></div><div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {campaigns.map(c => (<div key={c.id} className="border border-gray-200 rounded-lg p-4"><h3 className="font-semibold text-gray-900 mb-2">{c.title}</h3><p className="text-sm text-gray-600 mb-3">{c.description}</p><div className="flex justify-between items-center text-xs text-gray-500"><span>Tipo: {c.target_blood_type}</span><span className={`px-2 py-1 rounded-full ${c.is_active ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'}`}>{c.is_active ? 'Ativa' : 'Inativa'}</span></div></div>))}
        </div></div>
      </main>
      <Modal isOpen={isCampaignModalOpen} onClose={() => setIsCampaignModalOpen(false)} title="Criar Nova Campanha">
        <div className="space-y-4">
          ...
          <div className="flex justify-end space-x-4"><button onClick={() => setIsCampaignModalOpen(false)} className="px-4 py-2 border rounded-lg">Cancelar</button><button onClick={createCampaign} className="px-4 py-2 bg-primary-600 text-white rounded-lg">Criar</button></div>
        </div>
      </Modal>
